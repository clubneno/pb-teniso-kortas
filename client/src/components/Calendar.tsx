          ...
          {calendarDays.map((calDay, index) => {
            ...
            return (
              <button
                key={index}
                className={`
                  text-center py-2 rounded-lg transition-all duration-200
                  ${isSelected ? 'bg-tennis-green-500 !text-white font-black text-base border-2 border-tennis-green-300 shadow-lg shadow-tennis-green-500/30 scale-105 ring-2 ring-tennis-green-200' : 'text-sm'}
                  ${!calDay.isCurrentMonth && !isSelected ? 'text-gray-400' : ''}
                  ...
                  ${!isPast && calDay.isCurrentMonth && !isSelected && !isTodayDate ? 'text-gray-700 hover:bg-tennis-green-50 hover:scale-105' : ''}
